...
def write_html_doc(functions, output_path, arguments, todo_stats):
    ...
    This is the main-writing functions.
    """
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        write_html_header(f, document_meta)
        if show_progress:
            write_html_progress_section(f, todo_stats)
        write_html_toc(f, functions, highlight_todo)
        write_functions_to_html(f, functions, highlight_todo)
        write_html_footer(f, arguments)

def write_html_header(f, document_meta):
    """
    Writing html header to report
    """
    logo_html = ""
...
def write_html_toc(f, functions, highlight_todo):
    """
    Writing html table of content to report
    """
    f.write("""
...
def write_html_progress_section(f, todo_stats):
    """
    Writing html overall documentation progress to report
    """
    f.write(f"""
...
""")
    ...
    # Writing additional statistic information to file
    f.write(f"""
    ...
    <div style="font-size: 0.9em; color: #555; max-width: 400px;">
        ...
        <div style='background-color: #eee; border-radius: 3px; overflow: hidden; height: 14px; margin-bottom: 10px;'>
            <div style='
                ...
                width: {todo_stats["percent_brief_done"]}%;
                height: 100%;
                text-align: right;
                padding-right: 4px;
                color: white;
        ...
        <div style='background-color: #eee; border-radius: 3px; overflow: hidden; height: 14px; margin-bottom: 10px;'>
            <div style='
                ...
                width: {todo_stats["percent_tparams_done"]}%;
                height: 100%;
                text-align: right;
                padding-right: 4px;
                color: black;
        ...
        <div style='background-color: #eee; border-radius: 3px; overflow: hidden; height: 14px; margin-bottom: 10px;'>
            <div style='
                background-color: #ffc107;
                width: {todo_stats["percent_params_done"]}%;
                height: 100%;
                text-align: right;
                padding-right: 4px;
                color: black;
        ...
        <div style='background-color: #eee; border-radius: 3px; overflow: hidden; height: 14px; margin-bottom: 10px;'>
            <div style='
                ...
                width: {todo_stats["percent_return_done"]}%;
                height: 100%;
                text-align: right;
                padding-right: 4px;
                color: white;
...
def write_functions_to_html(f, functions, highlight_todo):
    """
    Writing html function-block to report
    """
    for idx, func in enumerate(functions):
        ...
        if todo_marker == "❌ ":
            f.write("""
                <div style='
                    ...
                    padding: 10px;
                    border-radius: 4px;'>
                    ⚠️ <strong>Warning:</strong> Function contains a TODO mark!
                </div>
            """)
        ...
        if func.get("doxygen"):
            write_comment_to_html(f, func, idx)

def write_comment_to_html(f, func, idx):
    """
    Writing doxygen-based function comment to html report
    """
...
def write_html_footer(f, arguments):
    """
    Writing html footer to report
    """
    f.write(f"""
